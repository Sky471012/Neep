...
exports.sendOtp = async (req, res) => {
  ...
  try {
    ...
    await OtpLog.create({ email, otp, expiresAt: Date.now() + 5 * 60 * 1000 });
...
exports.verifyOtp = async (req, res) => {
  ...
  try {
    const log = await OtpLog.findOne({ email, otp, expiresAt: { $gt: new Date() } });
    if (!log) return res.status(400).json({ message: 'Invalid or expired OTP' });
